	...
	const experienceData = [
		{
			...
			endDate: "Present",
			img: "/ca.jpeg",
			desc: "Working on the Content-Hub as a backend engineer",
		},
		{
		...
		<div className="flex flex-col-reverse md:flex-row gap-4">
			<div>
				...
				<ul className="my-4 flex gap-4 items-center ">
					{socials.map((social, idx) => (
						<li key={idx}>
							<LinkPreview
								className="flex gap-0.5 items-end cool-underline text-background"
								url={social.link}
							>
			...
			<div className="w-64 h-64 relative flex-shrink-0">
				<Image
					...
					fill
					alt="profile"
					className="rounded-md object-cover object-top pixel-border"
				/>
			</div>
		</div>
	);
